...
- 极速击键打反与拼写错误智能纠错（Typo Correction）：参考主流成熟输入法机制，针对快速打字时常见的击键时序颠倒与音节拼写误触进行精准识别与重构（如将 `gn` 自动纠正为 `ng`，输入 `yuedign` 精准切分为 `yue'dign` 并自动置顶首选“约定”，`xiagn` -> “向/想”，`shuei` -> “水”，`jiou` -> “九”，`luen` -> “论”，`lve` -> “略”等）；Preedit 优雅保留用户击键原文并正确划分隔音符，原始编码沉底供回车上屏；采用纯原生纳秒级字符流扫描与全局异常隔离防线，在高频极限极速击键下坚如磐石、零闪退；
