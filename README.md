...
设置客户端按功能分为基本设置、候选设置、剪贴板、智能学习、用户词典和输入法状态六页：

- 修改后可选择“应用”或“保存并关闭”；关闭窗口时会提示未保存修改；
- 基本设置页可以开启中文标点全角转换（，。？！等，英文模式自动保持半角）；
- 剪贴板页可以搜索、选择、复制、删除和清空历史，清空操作需要确认；
- 智能学习页显示当前记录数量，清空前会自动备份并验证备份；
- 用户词典页支持实时搜索、添加、编辑、删除、导入和导出；导入文件必须完整通过校验，失败不会覆盖现有词典；
- 输入法状态页分别显示 `fcitx5-remote`、Fcitx5、当前输入法和 ModernIME 的状态，并提供“刷新状态”和“重新加载 ModernIME”。

所有数据管理操作都只作用于当前用户目录。修改配置或词典后，使用状态页的“重新加载 ModernIME”即可让运行中的 Fcitx5 重新读取配置。
