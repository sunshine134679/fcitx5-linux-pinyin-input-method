...
ModernIME 是一个面向 Linux 桌面的 Fcitx5 拼音输入法，包含候选栏、用户习惯学习、用户词典、剪贴板入口，以及独立的 GTK3 设置客户端。
...
- 输入体验页统一配置启用状态、默认中英文状态、切换快捷键、中文标点和候选操作；快捷键格式错误会在对应输入框就地提示；
...
ModernIME 托管数据保存在当前用户配置目录；词典导入与导出使用用户在文件选择器中指定的路径。第一阶段的设置客户端不会发起任何网络请求，也不包含联网入口或后台联网任务。修改配置或词典后，使用系统与诊断页的“重新加载 ModernIME”即可让运行中的 Fcitx5 重新读取配置。
...
- 中文输入：输入拼音后使用数字键选择候选，候选列表支持翻页和方向键操作；
- 动态分页：候选栏保持固定宽度，并按本页候选文字长度动态决定可显示数量；翻页、方向键和数字键共享连续索引，不会跳过候选；
...
- 拼音恢复：常见模糊音和拼写颠倒可恢复中文候选，可信的高频三字母简拼可直接显示常用短语，同时普通英文仍保留原文优先；
- 英文输入：在中文输入状态输入非拼音英文串时保留原始英文候选；
- 剪贴板：在中文输入状态输入触发字母（默认 `v`）后紧跟触发数字（默认 `2`）即可打开剪贴板列表；触发字母本身会正常进入拼音组合，不会被吞掉或改写；
