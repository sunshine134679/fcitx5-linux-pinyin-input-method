...
│
└── tests/                      # CTest 自动化测试套件 (47 个全量用例，必须保持 100% 通过)
...
### 🚨 铁律三：测试全绿方可交付
- 仓库内包含 47 个全量测试（涵盖基础逻辑、拼音引擎、标点、GTK契约、安装运行测试）。
- 任何代码改动必须执行 `ctest --test-dir build/fcitx5-debug --output-on-failure`，必须确保 **47/47 全部 Passed**。
